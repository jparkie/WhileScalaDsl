...
case class WhileAndOperator(leftArgument: WhileBooleanType, rightArgument: WhileBooleanType) extends WhileBooleanType {
  override def nodeName: String = classOf[WhileAndOperator].getSimpleName
  ...
  override def asWhile: Try[String] = {
    ...
    } yield s"$leftArgumentWhile ${WhileKeywords.`AND`} $rightArgumentWhile"
  }
